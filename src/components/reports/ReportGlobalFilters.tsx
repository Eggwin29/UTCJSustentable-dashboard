...
export default function ReportGlobalFilters({
  ...
  return (
    <Card variant="outlined">
      <Card.Header className="p-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <button
            ...
            aria-controls="report-global-filter-content"
            onClick={() =>
              setIsOpen(
                (current) => !current
              )
            }
            className="group flex min-w-0 flex-1 items-center gap-3 rounded-xl text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-800"
          >
            <span className="shrink-0 rounded-xl bg-emerald-100 p-2.5 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">
              <FiFilter
                size={20}
                aria-hidden="true"
              />
            ...
            <span className="min-w-0 flex-1">
              ...
              <span className="mt-0.5 block text-sm text-slate-500 dark:text-slate-400">
                {isOpen
                  ? "Configura el periodo y las selecciones específicas."
                  : getCollapsedGlobalSummary(
                      activeLabels,
                      error
                    )}
              </span>
            ...
            <span className="flex shrink-0 items-center gap-2 text-sm font-medium text-slate-500 transition-colors group-hover:text-emerald-700 dark:text-slate-400 dark:group-hover:text-emerald-400">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2 pl-13 lg:pl-0">
            <Badge
              variant={
                activeFilterCount > 0
                  ? "primary"
      ...
      {isOpen && (
        <Card.Body
          id="report-global-filter-content"
          className="space-y-5 border-t border-slate-200 pt-5 dark:border-slate-700"
        >
          <div>
...
function getCollapsedGlobalSummary(
  activeLabels: string[],
  error: Error | null
): string {
  if (error) {
    return "No fue posible cargar las opciones de filtrado.";
  }

  if (activeLabels.length === 0) {
    return "Vista histórica completa, sin restricciones. Selecciona para configurar.";
  }
